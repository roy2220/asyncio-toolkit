...
_Py_IDENTIFIER(_schedule_immediate_callbacks);
...
#define FutureObj_HEAD(prefix)                                              \
    ...
    PyObject *prefix##_immediate_callbacks;                                 \
...
static inline int future_call_schedule_immediate_callbacks(FutureObj *);
...
static int
future_schedule_immediate_callbacks(FutureObj *fut)
{
    Py_ssize_t len;
    PyObject *callbacks;
    int i;

    if (fut->fut_immediate_callbacks == NULL) {
        PyErr_SetString(PyExc_RuntimeError, "uninitialized Future object");
        return -1;
    }

    len = PyList_GET_SIZE(fut->fut_immediate_callbacks);
    if (len == 0) {
        return 0;
    }

    callbacks = PyList_GetSlice(fut->fut_immediate_callbacks, 0, len);
    if (callbacks == NULL) {
        return -1;
    }
    if (PyList_SetSlice(fut->fut_immediate_callbacks, 0, len, NULL) < 0) {
        Py_DECREF(callbacks);
        return -1;
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        res = PyObject_CallFunctionObjArgs(cb, fut, NULL);
...
future_init(FutureObj *fut, PyObject *loop)
{
    ...
    Py_CLEAR(fut->fut_immediate_callbacks);
    ...
    fut->fut_immediate_callbacks = PyList_New(0);
    if (fut->fut_immediate_callbacks == NULL) {
        return -1;
    }
...
future_set_result(FutureObj *fut, PyObject *res)
{
    ...
    if (future_call_schedule_immediate_callbacks(fut) == -1) {
        return NULL;
    }
    Py_RETURN_NONE;
}
...
static PyObject *
future_add_immediate_done_callback(FutureObj *fut, PyObject *arg)
{
    if (fut->fut_state != STATE_PENDING) {
        PyObject *handle = _PyObject_CallMethodIdObjArgs(fut->fut_loop,
                                                         &PyId_call_soon,
        ...
        int err = PyList_Append(fut->fut_immediate_callbacks, arg);
        if (err != 0) {
            return NULL;
        }
    }
...
FutureObj_traverse(FutureObj *fut, visitproc visit, void *arg)
{
    ...
    Py_VISIT(fut->fut_immediate_callbacks);
...
static PyObject *
_asyncio_Future_add_immediate_done_callback(FutureObj *self, PyObject *fn)
{
    return future_add_immediate_done_callback(self, fn);
}
...
static PyObject *
_asyncio_Future_remove_immediate_done_callback(FutureObj *self, PyObject *fn)
{
    PyObject *newlist;
    Py_ssize_t len, i, j=0;

    if (self->fut_immediate_callbacks == NULL) {
        PyErr_SetString(PyExc_RuntimeError, "uninitialized Future object");
        return NULL;
    }

    len = PyList_GET_SIZE(self->fut_immediate_callbacks);
    if (len == 0) {
        return PyLong_FromSsize_t(0);
    }
    ...
    for (i = 0; i < PyList_GET_SIZE(self->fut_immediate_callbacks); i++) {
        int ret;
        PyObject *item = PyList_GET_ITEM(self->fut_immediate_callbacks, i);
        Py_INCREF(item);
        ret = PyObject_RichCompareBool(fn, item, Py_EQ);
        if (ret == 0) {
            if (j < len) {
    ...
    len = PyList_GET_SIZE(self->fut_immediate_callbacks);
    if (j != len) {
        if (PyList_SetSlice(self->fut_immediate_callbacks, 0, len, newlist) < 0) {
            goto fail;
        }
    }
    Py_DECREF(newlist);
...
static PyObject *
FutureObj_get_immediate_callbacks(FutureObj *fut)
{
    if (fut->fut_immediate_callbacks == NULL) {
        Py_RETURN_NONE;
    }
    Py_INCREF(fut->fut_immediate_callbacks);
    return fut->fut_immediate_callbacks;
}
...
static PyObject *
_asyncio_Future__schedule_immediate_callbacks_impl(FutureObj *self)
{
    int ret = future_schedule_immediate_callbacks(self);
    if (ret == -1) {
        return NULL;
    }
    Py_RETURN_NONE;
...
static PyMethodDef FutureType_methods[] = {
    ...
    _ASYNCIO_FUTURE_ADD_IMMEDIATE_DONE_CALLBACK_METHODDEF
    _ASYNCIO_FUTURE_REMOVE_DONE_CALLBACK_METHODDEF
    _ASYNCIO_FUTURE_REMOVE_IMMEDIATE_DONE_CALLBACK_METHODDEF
    _ASYNCIO_FUTURE_CANCEL_METHODDEF
    _ASYNCIO_FUTURE_CANCELLED_METHODDEF
    _ASYNCIO_FUTURE_DONE_METHODDEF
    _ASYNCIO_FUTURE__REPR_INFO_METHODDEF
    _ASYNCIO_FUTURE__SCHEDULE_CALLBACKS_METHODDEF
    _ASYNCIO_FUTURE__SCHEDULE_IMMEDIATE_CALLBACKS_METHODDEF
    {NULL, NULL}        /* Sentinel */
};
...
#define FUTURE_COMMON_GETSETLIST                                                     \
    ...
    {"_immediate_callbacks", (getter)FutureObj_get_immediate_callbacks, NULL, NULL}, \
...
static inline int
future_call_schedule_immediate_callbacks(FutureObj *fut)
{
    if (Future_CheckExact(fut)) {
        return future_schedule_immediate_callbacks(fut);
    }
    else {
        /* `fut` is a subclass of Future */
        PyObject *ret = _PyObject_CallMethodId(
            (PyObject*)fut, &PyId__schedule_immediate_callbacks, NULL);
        if (ret == NULL) {
            return -1;
        }
...
/*[clinic end generated code: output=6bfc0479da9d5757 input=13f9bf496695cb52]*/
{
    ...
    if (self->task_fut_waiter) {
        ...
        if (((FutureObj *)self->task_fut_waiter)->fut_state == STATE_CANCELLED) {
            Py_RETURN_TRUE;
        }
...
static PyMethodDef TaskType_methods[] = {
    ...
    _ASYNCIO_TASK_CANCELING_METHODDEF
...
task_step_impl(TaskObj *task, PyObject *exc)
{
    ...
    if (task->task_fut_waiter) {
        if (!task->task_must_cancel && Task_CheckExact(task->task_fut_waiter)) {
            task->task_must_cancel = ((TaskObj *)task->task_fut_waiter)->task_must_cancel;
        }
